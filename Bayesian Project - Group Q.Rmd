...
```{r JustAnExample, echo=FALSE, include=FALSE}
...
$$
\begin{split}
P(x_{k,j} < x_k < x_{k,j+1}) =\int_{x_{j}}^{x_{j+1}} \frac{1}{\Gamma(\phi_k^{-1})} \big( \phi_k \mu \big)^{\phi_k} x_k^{\frac{1}{\phi_k}-1} \exp{(\frac{-x_k}{\phi_k \mu})}
\end{split}
$$
...
& \approx \frac{1}{\Gamma(\phi_k^{-1})} \big( \phi_k \mu \big)^{\phi_k} x_j^{\frac{1}{\phi_k}-1} \exp{(\frac{-x_j}{\phi_k \mu})} \Delta_j
\end{split}
$$
...
$$
\bar{x}_k \sim N(\mu= 3000, \sigma =306) \propto \sigma^{-1/2} \exp{ \big(-\frac{1}{2\sigma}(x_k-\mu)^2\big) }
$$
