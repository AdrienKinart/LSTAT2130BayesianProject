...
```{r estim_parameters_plot, fig.show='hold', fig.width="49%"}
...
Based on http://www.sumsar.net/blog/2013/11/easy-laplace-approximation/
...
```{r laplace_approx_plot, fig.show='hold', fig.width="49%"}
plot(density(laplace_fl[, 1]), main = "laplace approximation for mu in Flanders")
plot(density(laplace_fl[, 2]), main = "laplace approximation for phi in Flanders")
```
...
(function(){
  ...
  x1 <- seq(2400, 3600, length = 5000)
  y1 <- seq(0.1, 0.4, length = 5000)
  
  z1 <- outer(x1, y1, function(x,y) {
    dmvnorm(cbind(x, y), colMeans(laplace_fl), cov(laplace_fl), log=T)
  })
  
  R <- exp(z1 - max(z1))
  
  plot(x = laplace_fl[, 1], y = laplace_fl[, 2], 
...
\begin{note}[Plot]
  Retiré. Voir comparaison section 5.3 (c)
\end{note}
