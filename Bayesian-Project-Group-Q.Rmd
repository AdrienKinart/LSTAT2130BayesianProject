...
A summary of the data generated is shown here below:
\begin{figure}[h]
\hspace{-2.5em}
\begin{BVerbatim}
...
$$
Accordingly, a 95\% quantile based or HPD based can be easily taken from the generated samples.  This is shown in the following graph where the marginal distribution of the mean for Flanders is plotted. 
...
```{r metropolis_algorithm, appendix=TRUE}
...
```{r acceptance_rate_metropolis_1, results='', appendix=TRUE}
# Check acceptance rate (aim : 40%)
...
plot.acf.pacf <- function(data, lag.max = 50, linked_by_line = T, titles = c("ACF", "PACF"), ...) {
...
```{r acf_pacf_mu, fig.show="hold", out.width="49%", fig.width=5.5, appendix=TRUE}
...
gelman.diag(list(mcmc(metro_fl1$theta), mcmc(metro_fl2$theta)))$psrf
...
```{r jags_1_traceplot, fig.show="hold", out.width="49%", fig.width=5.5, appendix=TRUE}
traceplot(out_fl)
...
```{r jags_2_traceplot, fig.show="hold", out.width="49%", fig.width=5.5, appendix=TRUE}
traceplot(out_wal)
...
\vspace{-2em}
```{r plot_metropolis_1_mu_phi, appendix=TRUE}
