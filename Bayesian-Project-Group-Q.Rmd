...
metro_model <- function() {
  ...
  # Priors
  mu ~ dnorm(3000, pow(306.12, -2)) # JAGS syntax: 2nd term is precision, which is 1/sd^2
  phi ~ dunif(0, 10)
}
```
...
out_wal <- coda.samples(model = jags_wal, c("mu", "phi"), n.iter = 50000) # want same chain length as in metropolis Flanders
...
out_wal2 <- coda.samples(model = jags_wal2, c("mu", "phi"), n.iter = 45000) # want same chain length as in metropolis Flanders
...
```
The density is again rather symmetric but centered on a smaller value (```r round(mean(out_wal.matrix[,1]),1)```). The exact values for the intervals in given below:
...
\toprule
                    & \multicolumn{2}{c|}{\textbf{Quantiles}} & \multicolumn{2}{c}{\textbf{HPD}} \\ \midrule
\textbf{Metropolis} & `r round(QB_ME_WAL[1], 2)` & `r round(QB_ME_WAL[2], 2)` & `r round(unlist(HPD_ME_WAL)[1], 2)` & `r round(unlist(HPD_ME_WAL)[2], 2)`             \\ \bottomrule
\end{tabular}
...
The detailed comparison is terms of values is detailed in te next question.
