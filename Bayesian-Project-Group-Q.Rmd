...
metropolis_algorithm = function(M, theta, sd.propositions, factors, frequencies){
  ...
  for (i in 2:(M+1)){
    ...
    thresholds = c(
      min(exp(lpost2(theta_mu, frequencies) - lpost2(this_theta, frequencies)),1),
      min(exp(lpost2(theta_phi, frequencies) - lpost2(this_theta, frequencies)),1)
    )
...
```{r jags_model_execution}
model.file = "resources/metropolis.bug"

# Test with Flanders
jags_fl = jags.model(file=model.file, 
                 inits=list(list(mu=3090, phi=0.4)), 
