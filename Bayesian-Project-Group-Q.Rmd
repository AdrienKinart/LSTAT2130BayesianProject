...
\end{split}
$$
By assuming the gamma distribution, one can plot their experimental histogram and plot the theoretical density plot by estimating the shape and rate parameters with Maximum of Likelihood. This allows to get a first sight on the data behaviour at hand.
...
hist(Fl, probability = T, xlim = c(-500,7000), main = TeX(r'(Histogram of Flanders and gamma distribution with parameters $\hat{\mu_{Fl}}$ and $\hat{\phi_{Fl}}$)'), xlab = "Flanders")
...
hist(Wall, probability = T, xlim = c(-500,7000), main = TeX(r'(Histogram of Wallonia and gamma distribution with parameters $\hat{\mu_{Wal}}$ and $\hat{\phi_{Wal}}$)'), xlab = "Wallonia")
...
## Question 3.a: Posterior for Flanders
...
fit_Fl <- optim(inits_Fl, lpost, control = list(fnscale = -1),
                hessian = TRUE, freq= Table[1,] )
...
plot(density(laplace_samples_Fl[,1]), main = TeX(r'(Laplace approximation for $\mu_{Fl}$)'))
plot(density(laplace_samples_Fl[,2]), main = TeX(r'(Laplace approximation for $\phi_{Fl}$)'))
```
...
To get the credible intervalfor $\mu$, we first need the marginal posterior distribution of $\mu$, this distribution is given by :

$$
P(\mu| D) \propto \int p(\mu, \phi |D) \diff \phi
$$
It can be shown that the marginal (univariate) distribution of the bivariate Gaussian distribution $N\big( \mu_\theta, \Sigma)$ also follow a normal distribution with paramters :
...
```
The interval based on the quantile is  [`r c(round(QuantileCI_Laplace_Fl[1],2), round(QuantileCI_Laplace_Fl[2],2))`] , while the quantile based on the highest posterior density is [`r c(round(HPDIntervals_Laplace_Fl[1],2), round(HPDIntervals_Laplace_Fl[2],2))`].
...
# Test with Flanders
jags_fl = jags.model(file=model.file, 
...
# Question 7: same but for Wallonia 
```{r jags_model_executionWaallonia}
model.file = "resources/metropolis.bug"

# Test with Wallonia
jags_wal = jags.model(file=model.file, 
...
# Question 8: Credible interval for means difference
...
```{r}
plot(density(out_fl.matrix[,"mu"]), main= "Comparison between Flanders and Wallonia Net Income households",
...
plot(density(diff_of_mu), main = "Difference of Net Income between Flanders and Wallonia",
     xlab = TeX(r'($\hat{\mu}_1 - \hat{\mu}_2$)'), cex.main= 0.9, lwd=2)
