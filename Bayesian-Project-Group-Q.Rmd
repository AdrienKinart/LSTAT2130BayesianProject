...
## Approximation to the joint posterior
...
In this context, we optimize the log-posterior to find the mode of the distribution, and we set the parameters `hessian = TRUE` to compute the variance-covariance matrix of the log-posterior distribution. Note that the variance-covariance matrix is find by taking the second derivative of the distribution.
...
plot(density(laplace_fl$samples[, 1]), main = "laplace approximation for mu in Flanders")
plot(density(laplace_fl$samples[, 2]), main = "laplace approximation for phi in Flanders")
```
...
The interval based on the quantile is [`r c(round(QB_LP[1],2), round(QB_LP[2],2))`] , while the quantile based on the highest posterior density is [`r c(round(HPD_LP[1],2), round(HPD_LP[2],2))`].
...
sd_hat_mu <- sd(laplace_fl$samples[,"mu"])
sd_hat_phi <- sd(laplace_fl$samples[,"phi"])
```
...
```{r}
par(mfrow = c(2, 1))
traceplot(as.mcmc(metro_fl1.theta))
```
...
```{r geweke_plot}
par(mfrow = c(2, 1))
geweke.plot(mcmc((metro_fl1.theta)), nbins = 50)
```
...
```{r jags_model_definition, eval=F}
