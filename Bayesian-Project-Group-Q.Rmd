...
lpost <- function(theta,freq){
  ...
  logPost <- LL -(1/(2*sigma_prior^2))*(mu- mu_prior)^2
  return(logPost)
}
```
...
M <- 55000
burnin <- floor(M/10) # after, should become stationary

MetropMu <- rep(NA,M)
MetropMu[1] <- Estimated_mu_Fl # MLE
n_accept_mu<-0 # it's a counter of the number of accepted samples
mu_factor <- 2.75 # multiply the var-cov matric ==> influence the acceptance ratethat we want to be 23%

MetropPhi <- rep(NA,M)
MetropPhi[1] <- Estimated_phi_Fl # MLE
n_accept_Phi<-0
phi_factor <- 2.75

for (i in 2:M){
  mu_prop <- MetropMu[i-1] + rnorm(1,0,mu_factor*sd_hat_of_mu)
  phi_prop <- MetropPhi[i-1] + rnorm(1,0,phi_factor*sd_hat_of_phi)
  
  theta_propMu <- c(mu_prop,MetropPhi[i-1]) #dernier disponible
  theta_propPhi <- c(MetropMu[i-1], phi_prop) #MetropMu[i-1] ou MetropMu[i] à utiliser? Le dernier est dispo en soit...
  
  theta_current <- c(MetropMu[i-1], MetropPhi[i-1])
  
  prob_mu <- min(1,exp(lpost(theta_propMu, freq = t(Table[1,]) )-lpost(theta_current, freq = t(Table[1,]))))
  accept_mu <- (runif(1)<=prob_mu)
  if (accept_mu){
    MetropMu[i] <- mu_prop
    n_accept_mu <- n_accept_mu + 1
  } else {
    MetropMu[i] <- MetropMu[i-1]
  }
  
  prob_phi <- min(1,exp(lpost(theta_propPhi, freq = t(Table[1,]) )- lpost(theta_current, freq = t(Table[1,]))))
  accept_phi <- (runif(1)<=prob_phi)
  if (accept_phi){
    MetropPhi[i] <- phi_prop
    n_accept_Phi <- n_accept_Phi + 1
  } else {
    MetropPhi[i] <- MetropPhi[i-1]
  }
  
}

theta_before_removing_burning <- cbind(MetropMu, MetropPhi)

# Exclude burnin
MetropMu = MetropMu[-c(1:burnin)]
MetropPhi = MetropPhi[-c(1:burnin)]

theta <- cbind(MetropMu, MetropPhi)
accept_rate_mu <- paste0(round(n_accept_mu/(M-1),digits=2)*100,"%")
cat("Acceptance rate for mu: ",accept_rate_mu,"\n") 

accept_rate_phi <- paste0(round(n_accept_Phi/(M-1),digits=2)*100,"%")
cat("Acceptance rate for phi: ",accept_rate_phi,"\n") 

```

```{r}
plot(MetropMu, MetropPhi, xlab = 'mu', ylab = 'phi')
library(MASS)
library(plotly)
den3d <- kde2d(theta[,1], theta[,2])
...
pacf((theta[,1]), lag.max = 30)
acf((theta[,2]), lag.max = 30)
pacf((theta[,2]), lag.max = 30)
```
Since the chains clearly seem to be generated with respect to an Autoregressive process (see ACF/PACF plots), one can compute effectively the effective sample size. It represents how many samples would be generated if there were no autcorrelation. 
```{r}
effectiveSize(theta)
```
...
#### Gelman-Rubin 
This method requires multiple chains. In this work, the diagnostic will be done with 2 generated chains with two different starting values. 
...
```{r}
geweke.plot(mcmc((theta)), nbins = 50)
```
