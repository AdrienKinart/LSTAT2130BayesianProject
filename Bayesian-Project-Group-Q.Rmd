...
sd_prop_mu <-1.6 # multiply the var-cov matric ==> influence the acceptance ratethat we want to be 23%
# ==> se trouve via iterations dans la boucle du bas jusqu'à avoir le bon acceptance rate. 

MetropPhi <- rep(NA,M)
MetropPhi[1] <- Estimated_phi_Fl # MLE
n_accept_Phi<-0
sd_prop_Phi < 0.0000001


for (i in 2:M){
  mu_prop <- MetropMu[i-1] + rnorm(1,0,sd_prop_mu^2*sd_hat_of_mu)
  theta_prop <- c(mu_prop,MetropPhi[i-1]) #dernier disponible
  theta_current <- c(MetropMu[i-1], MetropPhi[i-1])
  prob_mu <- min(1,exp(lpost(theta_prop, freq = t(Table[1,]) )-lpost(theta_current, freq = t(Table[1,]))))
  ...
  phi_prop <- MetropPhi[i-1] + rnorm(1,0,sd_prop_Phi^2*sd_hat_of_phi)
  theta_prop <- c(MetropMu[i-1], phi_prop) #MetropMu[i-1] ou MetropMu[1] à utiliser? Le dernier est dispo en soit...
  theta_current <- c(MetropMu[i-1], MetropPhi[i-1]) # même question
  prob_phi <- min(1,exp(lpost(theta_prop, freq = t(Table[1,]) )- lpost(theta_current, freq = t(Table[1,]))))
