...
inits <- c(mu = mu_prior, phi = 0.01) # Starting values
...
metropolis_algorithm <- function(M, theta, sd.propositions, factors, frequencies) {
  ...
  for (i in 2:(M + 1)) {
    ...
    # Update if accepted, otherwise use the previous values
    thetas[i, ] <- this_theta
...
```
\end{BVerbatim}
\caption*{Effective sample size of Metropolis in Flanders}
\end{figure}
...
```{r traceplot_both_metro_wal, fig.show="hold", out.width="49%", fig.width=5.5, appendix=TRUE}
...
Accordingly, one cannot reject the null hypàothesis of same average parameters value on a 95\% confidence level. 
\newline
...
Except for some values, the hypothesis of same mu and phi are not rejected. There is still no proof of non convergence of the generated chains. 
...
JAGS allows to generate samples from a joint posterior by providing the distributions of the members of this element: first the likelihood and then the conjugate prior. The former is a multinomial distribution where the probability vector is composed of a difference of gamma CDFs. The latter is composed of a normal distribution (mean 3000 and standard deviation 306.12) for the $\mu_k$ and a uniform distribution (with bonds from $0$ to $10$) for coefficient of dispersion $\phi_k$.
Those have been defined earlier and are proposed here below:
...
metro_model <- function() {
  ...
  # Priors
  mu ~ dnorm(3000, pow(306.12, -2)) # 2nd term is precision, 1/sd^2
  phi ~ dunif(0, 10)
}
```
...
\toprule
              & \multicolumn{2}{c|}{\textbf{Quantiles}} & \multicolumn{2}{c}{\textbf{HPD}} \\ \midrule
\textbf{JAGS} & `r round(QB_JA_FL[1], 2)` & `r round(QB_JA_FL[2], 2)` & `r round(HPD_JA_FL[1], 2)` & `r round(HPD_JA_FL[2], 2)`          
\end{tabular}
...
The model is exactly the same as in Flanders since the the proposed (log-)posterior distribution is the same and the prior parameters is also proposed to be the same. 
Then, one needs to fill in the data specific to Wallonia observations from the study and its starting values. Those are proposed to be the MLE's of $\mu_2$ and $\phi_2$ (3043 and 0.243, respectively).
...
```{r wallonia_metropolis_traceplot, fig.height=5, out.width="49%", fig.show='hold', appendix=TRUE}
traceplot(out_wal)
```
...
```{r wallonia_acf_pacf, fig.show="hold", out.width="49%", fig.height=4, fig.width=5.5, appendix=TRUE}
plot.acf.pacf(out_wal.matrix[, 1], titles = c("AFC of mu for Wallonia", "PACF of mu for Wallonia"))
plot.acf.pacf(out_wal.matrix[, 2], titles = c("AFC of phi for Wallonia", "PACF of phi for Wallonia"))
...
\hspace{-3em}
\begin{BVerbatim}
```{r effective_size_metro_out_wal, results='asis', appendix=TRUE}
effectiveSize(out_wal)

```
\end{BVerbatim}
\caption*{Effective sample size of Metropolis in Wallonia}
\end{figure}


### Gelman-Rubin {-}

The two proposed starting values for $\mu_2$ and $\phi_2$ are the same as in Flanders section : $4000$ for $\mu_1$ and $0.1$ for $\phi_1$. After having run the second chain, the traceplot of both chain is plotted with respect to the first 1000 generations after the burn-in.

```{r jags_model2_execution_wallonia, appendix=TRUE}
...
\hspace{-3em}
\begin{BVerbatim}
```{r gelman_diag_wallonia, results='asis', appendix=TRUE}
...
\vspace{-1em}
```{r gelman_plot_wal, appendix=TRUE, fig.height=3.8, out.width="80%"}
gelman.plot(list(
...
The Z-scores are given given here below for $\mu_2$ and $\phi_2$

\begin{figure}[h]
\centering
\hspace{-3em}
\begin{BVerbatim}
```{r geweke_diag_wal, results='asis'}
...
If one plots the Z-score successively with repect to the number of iterations, this gives:

```{r geweke_plot_wal, appendix=TRUE, fig.show='hold', fig.height=3.8, out.width="80%"}
# Z-Score plot
...
## (c) Credible intervals for $\mu_2$

 
\vspace{-1em}
```{r credible_intervals_wal, appendix=TRUE}
# Credible intervals of the Metropolis
QB_JA_WAL <- quantile(out_wal.matrix[, 1], probs = c(alpha / 2, 1 - alpha / 2))
HPD_JA_WAL <- HPDinterval(out_wal[,1], prob = 1 - alpha)

densplot(out_wal[,1], main = "Estimated density of mu in Wallonia", xlab="Income",
         lwd=2, sub = expression(paste(alpha, " = ", 0.05)))
legend("topleft", legend= c("Quantiles Interval", "HPD interval"), cex = 0.8,
       col = c('red', 'blue'), lwd=2, lty = 2)

abline(v=QB_JA_WAL, col='red', lty=2, lwd=2)
abline(v=unlist(HPD_JA_WAL), col='blue', lty=2, lwd=2)

```
The density is again rather symmetric but centered on a smaller value (```r round(mean(out_wal.matrix[,1]),1)```). The exact values for the intervals are given below:
...
\toprule
                    & \multicolumn{2}{c|}{\textbf{Quantiles}} & \multicolumn{2}{c}{\textbf{HPD}} \\ \midrule
\textbf{Metropolis} & `r round(QB_JA_WAL[1], 2)` & `r round(QB_JA_WAL[2], 2)` & `r round(unlist(HPD_JA_WAL)[1], 2)` & `r round(unlist(HPD_JA_WAL)[2], 2)`             \\ \bottomrule
\end{tabular}
\caption{Comparison of credible intervals in Wallonia}
\label{credible_intervals_wallonia}
\end{table}


The detailed comparison is terms of values is presented on the next question.
...
abline(v = QB_DIFF, col = "red", lty = 2, lwd = 3)
abline(v = HPD_DIFF, col = "blue", lty = 2, lwd = 2)

```
On a $95\%$ confidence level, it is impossible to conclude that the average household income level is significantly different between the regions. According to this Bayesian approach, one can only be up to  ```r round((1-ecdf(diff_of_mu)(0)*2)*100 ,2)``` \% sure that the Flemish income is higher than in Wallonia. This is computed by looking from which confidence level one can reject the null hypothesis of same average income.
...
\pagebreak
### Summary of the JAGS model for Wallonia{#S3}
