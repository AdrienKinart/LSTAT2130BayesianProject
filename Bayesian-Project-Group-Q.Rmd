...
$\hat\mu$  & `r estim_mu_fl` & `r estim_mu_wal` &  &  \\
$\hat\phi$ & `r estim_phi_fl` & `r estim_phi_wal` &  &  \\
\end{tabular}
\caption {Estimated $\mu$ and $\phi$ for Flanders and Wallonia}
\end{table}

```{r estim_parameters_plot, fig.show='hold', out.width="49%"}
...
Assuming the frequency distribution in a given region to be multinomial,

one has, writing \(P:=(p{1},..,p_{10})\) and \(Y:= (Y_{1},... Y_{10})\):
    \begin{align*}
...
\begin{note}
  ...
  The idea of using a normal distribution is not unreasonable. Indeed, if one generate by monte-carlo simulations a large number of samples, it is possible to observe that the mean seems normally distributed.  
...
Laplace approximation is a method that allows to approximate the log posterior distribution $h(\mu_1,\phi_1, Y_1)$ by a multimodal normal distribution (bimodal in our case), using the second Taylor approximation. More precisely, it locate the mode of the log-posterior distribution, and use it as the mean of the normal distribution, and then, it derives the variance of the normal distribution by evaluating the curvature of the posterior at the mode.

\begin{note}
  Refaire un peu ce paragraphe ? 
\end{note}

In this context, we optimize the log-posterior to find the mode of the distribution, and we set the parameters `hessian = TRUE` to compute the variance-covariance matrix of the log-posterior distribution. Note that the variance-covariance matrix is found by taking the second derivative of the distribution.
...
Thus, we find the distribution of $\theta_1$ for Flanders, and the plot of their distribution are given by :

```{r laplace_approx_plot, fig.show='hold', out.width="49%"}
...
To get the credible interval for $\mu$, we first need the marginal posterior distribution of $\mu$, this distribution is given by :

$$
P(\mu| D) \propto \int p(\mu, \phi |D)\ \diff \phi
$$

It can be shown that the marginal (univariate) distribution of the bivariate Gaussian distribution $N\big( \mu_\theta, \Sigma)$ also follow a normal distribution with parameters :
...
We needs that $95\%$ of the marginal posterior to fall into the interval for Flanders, to do so, 
...
legend("topleft", legend=c("HPD", "Quantiles"),
...
The interval based on the quantile is [`r c(round(QB_LP[1],2), round(QB_LP[2],2))`] , while the quantile based on the highest posterior density is [`r c(round(HPD_LP[1],2), round(HPD_LP[2],2))`]. Those are really close.
...
metro_fl1 <- metropolis_algorithm(5e4,
  ...
  sd.propositions = c(mu = sd_hat_mu, 
                      phi = sd_hat_phi),
  factors = c(mu = 2.74, phi = 2.75),
...
```{r acceptance_rate_metropolis_1, results=''}
cat("Acceptance rate for mu: ", metro_fl1$accept_rate["mu"] * 100, "% \n")
cat("Acceptance rate for phi: ", metro_fl1$accept_rate["phi"] * 100, "% \n")
```
...
```{r, fig.height=5, out.width="49%", fig.show='hold'}
...
```{r acf_pacf_function}
# LiochePack::ts.plot.acf.pacf()

plot.acf.pacf = function(data, lag.max = 50, simplify=T, linked_by_line=T, titles=c("ACF", "PACF"), ...){
  ...
  if(simplify){
    ...
    ts.seq = seq(from=frequency(data), to=lag.max, by=frequency(data))
...
```{r acf_pacf_mu, fig.show="hold", out.width="49%", fig.width=5.5}
plot.acf.pacf(metro_fl1.theta[, 1], lag.max = 30)
plot.acf.pacf(metro_fl1.theta[, 2], lag.max = 30)
```
...
\hspace{-3em}
\begin{BVerbatim}
```{r effective_size_metro_1, results='asis'}
...
```{r traceplot_both_metro, fig.show="hold", out.width="49%", fig.width=5.5}
traceplot(list(
...
credible_intervals_comparison = function(alpha){
  ...
  densplot(laplace_fl.mcmc[, 1], main = "Laplace approximation for mu in Flanders")
  legend("topright",
...
```{r jags_model_definition, eval=F, echo=T}
...
```{r jags_model_execution_flanders}
model.file <- "resources/metropolis.bug"
...
```{r jags_1_traceplot, fig.show="hold", out.width="49%", fig.width=5.5}
traceplot(out_fl)
```
...
lines(density(metro_fl1.theta[, 1]), col = "red", lty = 2)
lines(density(laplace_fl$samples[, 1]), col = "blue", lty = 2)

plot(density(out_fl.matrix[, "phi"]), main = "Comparison R metropolis, Laplace and JAGS distribution for phi", lty = 2)
lines(density(metro_fl1.theta[, 2]), col = "red", lty = 2)
lines(density(laplace_fl$samples[, 2]), col = "blue", lty = 2)
```

# Question 7

```{r jags_model_execution_wallonia}
...
```{r jags_2_traceplot, fig.show="hold", out.width="49%", fig.width=5.5}
traceplot(out_wal)
```
...
(function(){
  ...
  plot(x = laplace_fl$samples[, 1], y = laplace_fl$samples[, 2], 
     xlab = "mu", ylab = "phi", pch = ".", col=rgb(0,0,0,.5))
  
  contour(x1, y1, R, lwd = 2, add=T,
