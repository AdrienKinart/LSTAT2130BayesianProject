...
metro_model = function(){
  ...
  # Priors
  mu ~ dnorm(3000, 1/pow(306.12, 2)) # in terms of tau (precision)
  phi ~ dunif(0, 10)
...
model.file = "metro_1.bug"
write.model(metro_model, model.file)

# Data and initial values for model parameters
mydata = list()
inits = list(list(mu=2915, phi=0.472))

# Model specification
foo = jags.model(file=model.file, inits=inits,data=mydata, n.chains=1)
update(foo, 1000)
