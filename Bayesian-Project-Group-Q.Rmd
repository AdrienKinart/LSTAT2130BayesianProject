...
#rownames(MetropMu) <- c("mu")
MetropPhi = MetropPhi[-c(1:burnin)]
#rownames(MetropPhi) <- "phi"
...
#   mu_prop <- MetropMu[i-1] + rnorm(1,0,mu_factor*sd_hat_of_mu)
#   theta_propMu <- c(mu_prop,MetropPhi[i-1]) #dernier disponible
#   theta_current1 <- c(MetropMu[i-1], MetropPhi[i-1])
# 
#   prob_mu <- min(1,exp(lpost(theta_propMu, freq = t(Table[1,]) )-lpost(theta_current1, freq = t(Table[1,]))))
#   accept_mu <- (runif(1)<=prob_mu)
#   if (accept_mu){
#     MetropMu[i] <- mu_prop
#     n_accept_mu <- n_accept_mu + 1
...
#   phi_prop <- MetropPhi[i-1] + rnorm(1,0,phi_factor*sd_hat_of_phi)
#   theta_propPhi <- c(MetropMu[i-1], phi_prop) #MetropMu[i-1] ou MetropMu[i] à utiliser? Le dernier est dispo en soit..
#   theta_current2 <- c(MetropMu[i], MetropPhi[i-1])
# 
#   prob_phi <- min(1,exp(lpost(theta_propPhi, freq = t(Table[1,]) )- lpost(theta_current2, freq = t(Table[1,]))))
#   accept_phi <- (runif(1)<=prob_phi)
#   if (accept_phi){
#     MetropPhi[i] <- phi_prop
#     n_accept_Phi <- n_accept_Phi + 1
...
```{r}
plot(theta[,1], theta[,2], xlab = 'mu', ylab = 'phi')
par(mfrow=c(2,2))
...
effectiveSize(mcmc((theta)))
```

Assessment from a single chain

```{r}
geweke.diag(mcmc((theta)))
par(mfrow=c(2,1))
geweke.plot(mcmc((theta)), nbins = 20)
```
...
HPDphi_metrop_Fl<- HPDinterval(as.mcmc((theta)))[2,]
