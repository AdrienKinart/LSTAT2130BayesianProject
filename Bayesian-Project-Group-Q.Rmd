...
\begin{equation}
\begin{split}
p_j & = F(x_{j+1}, \kappa, \lambda) - F(x_{j}, \kappa, \lambda) \\
& = \frac{1}{\Gamma(\kappa)} \Big ( \gamma(\kappa, \lambda x_{j+1}) -\gamma(\kappa, \lambda x_{j}) \Big) \\
& = \sum^{\kappa-1}_{i=0} \frac{\lambda x_j}{i !} \exp{(- \lambda x_j)} - \sum^{\kappa-1}_{i=0} \frac{\lambda x_{j+1}}{i !} \exp{(- \lambda x_{j+1})} \\
&=\lambda \exp{(-\lambda)}  \sum^{\kappa-1}_{i=0} \Bigg( \frac{x_j}{i!} \exp{(-x_j)} + \frac{x_{j+1}}{i!} \exp{(-x_{j+1})} \Bigg)
...
& \propto \sigma^{-1/2} \exp{ \big(-\frac{1}{2\sigma^2}(\bar{x}_k-\mu)^2\big) } 1_{0;10}
\end{split}
\end{equation}
$$
# Question 3
